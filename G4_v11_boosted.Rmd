...
We will put ourselves in the shoes of Government decision makers at different dates and analyze the public data available to come up with predictive models of mortality. 
...
The later prediction, being done 8 weeks after the lockdown date, should be a good indicator of the current situation of the crisis and of the level of control and certainty about the pandemic evolution. It will ultimately indicate whether lockdown shall be ended or pursued. 
...
Nevertheless, both first models would note take into account the lockdown effect, which should drastically change the spread of the virus and so further predictions. To do so, we would look at prediction 28 days after the lockdown start. 

We will finally take the latest data available (May 25th) to observe the latest predictions. By doing so, we will know the current status in each of the three countries.
This will give us insights on how the delay in lockdown dates for each country has affected the spread curve of the Coronavirus over 8 weeks. 
We will be able to compare different lockdown impacts on mortality prediction and will determine which countries are in good positions for ending the lockdown. Considering end of confinement also means economy restart, it would also give us a sense of which lockdown and stringency policy decisions were the most effective from an economic perspective. Indeed, the earlier the end of lockdown, the better for the economy. 
...
We will finally select the model that fits the best.

We will further reproduce the same process for France and UK. 
Finally we will present 3 best plots for each country
...
write.csv(cIT_14_AAND_pred, file = "IT no lockdown+14d (AAND) incl(ARIMA).csv") # export the selected model's predictions into a CSV file
```
...
cFRA_14 <- ts(log(covid_data$Cdeaths_IT[58:89]),start=t0, frequency=365) #t=0 @ day75. +14d 
cFRA_F <- ts(log(covid_data$Cdeaths_IT[58:148]),start=t0, frequency=365)   # a supprimer?
...
plot(cFRA_14_AAN_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cFRA_F, col="red")
plot(cFRA_14_MMN_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cFRA_F, col="red")
plot(cFRA_14_AAND_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cFRA_F, col="red")
plot(cFRA_14_MMND_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cFRA_F, col="red")
plot(cFRA_14_tbats_pred, xlab="Year", ylab="Predicted Electric Rate", ylim=c(0,100))
lines(cFRA_F, col="red")
plot(cFRA_14_ARIMA_pred, ylim=c(0,100))
lines(cFRA_F, col="red")
...
# Export the results out
write.csv(cFRA_14_AAND_pred, file = "FRA no lockdown+14d (AAND) incl(ARIMA).csv") # export the selected model's predictions into a CSV file
...
cUK_14 <- ts(log(covid_data$Cdeaths_IT[68:98]),start=t0, frequency=365) #t=0 @ day84. +14d 
cUK_F <- ts(log(covid_data$Cdeaths_IT[68:148]),start=t0, frequency=365)   # a supprimer?
...
par(mfrow=c(1,1))
plot(cUK_14_tbats_pred, xlab="Year", ylab="Predicted Electric Rate")
lines(cUK_F, col="red")

cUK_14_ARIMA <- auto.arima(cUK_14,seasonal=FALSE)
cUK_14_ARIMA_pred <- forecast(cUK_14_ARIMA,30)
plot(cUK_14_ARIMA_pred)
lines(cUK_F, col="red")

par(mfrow=c(3,2)) # This command sets the plot window to show 1 row of 4 plots
plot(cUK_14_AAN_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cUK_F, col="red")
plot(cUK_14_MMN_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cUK_F, col="red")
plot(cUK_14_AAND_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cUK_F, col="red")
plot(cUK_14_MMND_pred, xlab="Year", ylab="Acummulated deaths", ylim=c(0,100))
lines(cUK_F, col="red")
plot(cUK_14_tbats_pred, xlab="Year", ylab="Predicted Electric Rate", ylim=c(0,100))
lines(cUK_F, col="red")
plot(cUK_14_ARIMA_pred, ylim=c(0,100))
lines(cUK_F, col="red")
...
# Export the results out
write.csv(cUK_14_MMND_pred, file = "UK no lockdown+14d (MMND) incl(ARIMA).csv") # export the selected model's predictions into a CSV file
```
...
plot(cFRA_14_AAND_pred, xlab="Year", ylab="log(Acummulated deaths)", ylim=c(0,20))
lines(cFRA_F, col="red")
abline(v=2020+69/365, col="green")
legend("left", legend=c("Best fitted model","Real death toll", "Start of Lockdown"), col=c("blue", "red","green"), lty=1)

plot(cUK_14_MMND_pred, xlab="Year", ylab="log(Acummulated deaths)", ylim=c(0,20))
lines(cUK_F, col="red")
abline(v=2020+69/365, col="green")
legend("left", legend=c("Best fitted model","Real death toll", "Start of Lockdown"), col=c("blue", "red","green"), lty=1)
