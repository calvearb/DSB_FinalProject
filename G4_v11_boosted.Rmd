---
title: "Evaluation of lockdown effect in several European countries"
...
Government decisions for lockdown were mainly made by compromising between healthcare capabilities and economic disaster. The timing of governments' initiatives were often ambiguous and have been constantly being revised. 

Mitigate the economic impact of the Covid-19 and assess the stringency level of confinement have finally pushed governments to pronounce lockdowns at different moments. 
If all countries lacked experience in managing such a crisis, Italian lockdown decision was taken after patient 0 was discovered and after what we commonly call a "tipping point", that is when the virus had already spread fast. French decision was taken a week after Italy. Finally, United Kingdom, in political and economic trouble due to the Brexit situation, waited a week more than France to announce the confinement. 
...
Nevertheless, both first models would not take into account the lockdown effect, which should drastically change the spread of the virus and so further predictions. To do so, we would look at prediction 28 days after the lockdown start which should help measuring the assumed positive effect of lockdown. 
...
Decomposition model trigonometric BATS. TBATS will become BATS because of no seasonality so no need for trigonomectric adjustment. 
```{r, echo=FALSE}
par(mfrow=c(1,1)) # This command sets the plot window to show 1 row of 1 plots
...
```
In the 14 Days, the confidence interval is narrower. The models tend to be more precise and Italian and English are undervalued when compared to the real results.
On the other hand, the French predictions was more pessimistic than the actual results. The French very strict restrictions and control must have played a strong effect. 
...
```
After 28 days, Italian and English model are very stable and very closed to reality. The fact that they are liner explain the narrow confidence interval. While a boom was expected in France, the real curve remained unchanged, demonstrating superior performance in controlling the virus spread. 
...
```
After 8 weeks, much more data are available, and the lockdown effect is clearly seen as the three models are linear with almost the same results.
Overall, the prediction are much more accurate. 
...
In conclusion, we can conclude that the three countries have handled the Covid_19 crisis in different ways but with similar results in term of curves and potential relaunch of the economy in the same period of time after the lockdown.

While different models have been used to predict the death tolls at different period, linear will be the most accurate in the longer term as the situation is stabilized and over control.

The discrepancies in models can be explain by the abrupt switch of curve slope from exponential into a more linear one. The lockdown effectiveness is clearly demonstrated by this study.
