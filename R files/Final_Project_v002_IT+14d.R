...
cFRA2 <- ts(covid_data$Cdeaths_IT[53:83],start=2020+53/365, frequency=365) #t=0 @ day69. +14d
...
errors_AANF <- tsCV(cFRA2, f_AANF, h=1, window=24)

f_MMNF<- function(y, h) forecast(ets(y, model="MMN"), h = h)
errors_MMNF <- tsCV(cFRA2, f_MMNF, h=1, window=24)

f_AAND  <- function(y, h) forecast(ets(y, model="AAN", damped=TRUE), h = h)
errors_AAND <- tsCV(cFRA2, f_AAND, h=1, window=24)

f_MMND  <- function(y, h) forecast(ets(y, model="MMN", damped=TRUE), h = h)
errors_MMND <- tsCV(cFRA2, f_MMND, h=1, window=24)

f_TBATS  <- function(y, h) forecast(tbats(y), h = h)
errors_TBATS <- tsCV(cFRA2, f_TBATS, h=1, window=24)
...
write.csv(cFRA2_tbats_pred, file = "IT no lockdown+14d BATS.csv") # export the selected model's predictions into a CSV file
