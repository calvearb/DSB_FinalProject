...
cFRA2 <- ts(covid_data$Cdeaths_UK[66:83],start=2020+66/365, frequency=365) #t=0 @ day83. +0d
...
errors_AANF <- tsCV(cFRA2, f_AANF, h=1, window=16)

f_MMNF<- function(y, h) forecast(ets(y, model="MMN"), h = h)
errors_MMNF <- tsCV(cFRA2, f_MMNF, h=1, window=16)

f_AAND  <- function(y, h) forecast(ets(y, model="AAN", damped=TRUE), h = h)
errors_AAND <- tsCV(cFRA2, f_AAND, h=1, window=16)

f_MMND  <- function(y, h) forecast(ets(y, model="MMN", damped=TRUE), h = h)
errors_MMND <- tsCV(cFRA2, f_MMND, h=1, window=16)

f_TBATS  <- function(y, h) forecast(tbats(y), h = h)
errors_TBATS <- tsCV(cFRA2, f_TBATS, h=1, window=16)
...
write.csv(cFRA2_tbats_pred, file = "UK no lockdown+0d (BATS).csv") # export the selected model's predictions into a CSV file
