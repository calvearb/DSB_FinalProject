...
cFRA2 <- ts(covid_data$Cdeaths_FRA[45:131],start=2020+52/365, frequency=365) #t=0 @ day75. +58d
...
errors_AANF <- tsCV(cFRA2, f_AANF, h=1, window=31)

f_MMNF<- function(y, h) forecast(ets(y, model="MMN"), h = h)
errors_MMNF <- tsCV(cFRA2, f_MMNF, h=1, window=31)

f_AAND  <- function(y, h) forecast(ets(y, model="AAN", damped=TRUE), h = h)
errors_AAND <- tsCV(cFRA2, f_AAND, h=1, window=31)

f_MMND  <- function(y, h) forecast(ets(y, model="MMN", damped=TRUE), h = h)
errors_MMND <- tsCV(cFRA2, f_MMND, h=1, window=31)

f_TBATS  <- function(y, h) forecast(tbats(y), h = h)
errors_TBATS <- tsCV(cFRA2, f_TBATS, h=1, window=31)
...
write.csv(cFRA2_AAND_pred, file = "FRA no lockdown+56d (AAND).csv") # export the selected model's predictions into a CSV file

par(mfrow=c(1,1)) # This command sets the plot window to show 1 row of 4 plots
plot(cFRA2_AAND_pred, xlab="Year", ylab="Accumulated deaths", ylim=c(0,80000))
lines(cFRA3, col="red")
